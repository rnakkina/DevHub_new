...
node {
	...
	if (isUnix()){
		...
		bat "${toolbelt}"
		rc = bat returnstatus: true, script: "${toolbelt} "C:\Program Files\sfdx\bin\sfdx" auth:jwt:grant --clientid ${CONNECTED_APP_CONSUMER_KEY} --username ${HUB_ORG} --jwtkeyfile \"${jwt_key_file}\" --setdefaultdevhubusername --instanceurl ${SFDC_HOST}"
	}
